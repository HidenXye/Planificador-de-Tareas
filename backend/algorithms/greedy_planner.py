from .heap_queue import heap_sort_tareas
from .topological_sort import dependencias_resueltas
...
def greedy_plan(tareas, tiempo_disponible):
    ...
    while pendientes:
        candidatos = []
        for t in pendientes:
            if dependencias_resueltas(t, completadas_ids, {x.id for x in pendientes}):
